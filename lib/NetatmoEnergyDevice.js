...
module.exports = class NetatmoEnergyDevice extends NetatmoDevice {
  ...
  async onOAuth2Init() {
    ...
    if (this.hasCapability('netatmo_modulating_thermostat_mode')) {
      this.registerCapabilityListener('netatmo_modulating_thermostat_mode', this.onCapabilityNetatmoThermostatMode.bind(this));
    }
  }
  ...
  sync() {
    Promise.resolve().then(async () => {
      ...
      if (typeof room.therm_setpoint_mode === 'string' && this.hasCapability('netatmo_modulating_thermostat_mode')) {
        this.setCapabilityValue('netatmo_modulating_thermostat_mode', room.therm_setpoint_mode).catch(this.error);
      }
